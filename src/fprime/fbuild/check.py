...
from fprime.fbuild.target import TargetContext, TargetScope, ExecutableAction, CompositeTarget

from fprime.fbuild.target import BuildSystemTarget
from .enumerator import BuildTargetEnumerator, MultiBuildTargetEnumerator, RecursiveMultiBuildTargetEnumerator
...
class Check(ExecutableAction):
    ...
    def __init__(self, scope: TargetScope, build_target_enumerator: BuildTargetEnumerator = None):
    ...
    def execute_all(self, builder: "Build", context: TargetContext, args: Tuple[Dict[str, str], List[str], Dict[str, bool]]):
        ...
        if "-j" in make_args or "--jobs" in make_args:
            cli_args.extend(["--parallel", str(make_args.get("--jobs", make_args.get("-j", 1)))])
...
class CheckTarget(CompositeTarget):
    ...
    def __init__(self, scope: TargetScope, build_target_enumerators: List[BuildTargetEnumerator], *args, **kwargs):
        ...
        build_target = BuildSystemTarget(scope=scope, build_target_enumerator=build_target_enumerators[0], *args, **kwargs)
        check_action = Check(scope=scope, build_target_enumerator=build_target_enumerators[1])
        super().__init__(
            composite_targets=[build_target, check_action],
            scope=scope, build_target_enumerator=build_target_enumerators[0], *args, **kwargs)
